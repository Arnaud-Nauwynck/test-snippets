...
/**
 * 
 * see groovy.inspect.swingui.AstNodeToScriptVisitor
 */
public class JavaifierGroovyPrettyPrinterVisitor extends PrimaryClassNodeOperation implements GroovyCodeVisitor, GroovyClassVisitor {
    ...
    boolean showMetaClass = false;
    
    def JavaifierGroovyPrettyPrinterVisitor(Writer writer, boolean showScriptFreeForm = true, boolean showScriptClass = true) {
        this._out = writer
    ...
    void visitMethod(MethodNode node) {
        if (!showMetaClass && isMetaMethod(node)) {
            return;
        }
        node?.annotations?.each {
            visitAnnotationNode(it)
    ...
    protected boolean isMetaMethod(MethodNode node) {
        String n = node.getName();
        if (n.equals('$getStaticMetaClass') || n.equals('getMetaClass') || n.equals('setMetaClass')) {
            return true;
        }
        // cf also ambiguous method name in script object: "invokeMethod", "getProperty", "setProperty"  ...
        if (n.equals('invokeMethod') && node.getParameters().length == 2 && node.getParameters()[1].getName().equals('arguments')) {
            return true;
        }
        if (n.equals('getProperty') || n.equals('setProperty')) {
            return true;
        }
        return false;        
    }
    ...
    void visitField(FieldNode node) {
        if (!showMetaClass && matchMetaField(node)) {
            return;
        }
        node?.annotations?.each {
            visitAnnotationNode(it)
    ...
    protected boolean matchMetaField(FieldNode node) {
        String n = node.getName();
        if (n.equals('$staticClassInfo') || n.equals('__$stMC') || n.equals('metaClass')) {
    ...
    void visitBlockStatement(BlockStatement block) {
        block?.statements?.each {
            if (showMetaClass || ! matchAssignMetaClass(it)) {
                it.visit(this)
                printLineBreak()
            }
        }
    ...
    protected boolean matchAssignMetaClass(Statement stmt) {
        ...
        if (!be.getOperation().getText().equals("=")) return false;
        Expression left = be.getLeftExpression();
        if (!(left instanceof FieldExpression)) return false;
        FieldNode leftField = ((FieldExpression) left).getField();
        if (!leftField.getName().equals("metaClass")) return false;
