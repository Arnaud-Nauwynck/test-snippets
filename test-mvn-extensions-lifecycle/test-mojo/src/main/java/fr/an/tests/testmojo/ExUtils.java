...
public final class ExUtils {
	...
	public static String currentStackTraceShortPath() {
		...
		for (int i = 1; i < trace.length; i++) { // skip first (self) method
            ...
            if (isSkipTraceElt(traceElement)) {
        	    continue;
        	}
...
//        }
    ...
    private static boolean isSkipTraceElt( StackTraceElement te )
    {
        if (isSkipTraceEltClass(te.getClassName())) {
            return true;
        }
        return false;
    }

    public static boolean isSkipTraceEltClass(String className) {
        boolean res = false;
