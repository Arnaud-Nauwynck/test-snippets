...
function dropDirective($parse) {
  return {
    restrict: 'A',
    link: function (scope, element, attrs) {
      // console.log("dropDirective", {scope,element,attrs});
      var domElement = element[0];
      ...
      var dropAcceptHandler = $parse(attrs.dropAccept);
      ...
      function dragOverListener(event) {
          // console.log("event dragOver", event.dataTransfer);

          // PATCHED ... Check if drag event is accepted is accepted.  (should test if type are accepted)
          var isDropAccept = dropAcceptHandler(scope, { $event: event });
        if (! isDropAccept) {
            console.log("event dragOver ... !dropAccept($event) => return true");
            return true;
        }
