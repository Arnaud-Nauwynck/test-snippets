...
public enum ParquetEncoding {
	 ...
	 * DOUBLE - 8 bytes per value. IEEE. Stored as little-endian. BYTEARRAY - 4
	 * byte length stored as little endian, followed by bytes. FIXEDLENBYTEARRAY
	 * - Just the bytes.
	 */
	PLAIN(0),
	 ...
	 * the plain type. in a data page use RLEDICTIONARY instead. in a Dictionary
	 ...
	 * lengths are encoded using DELTABINARYPACKED
	 */
	DELTA_LENGTH_BYTE_ARRAY(6),

	/**
	 * Incremental-encoded byte array. Prefix lengths are encoded using
	 * DELTABINARYPACKED. Suffixes are stored as delta length byte arrays.
	 */
	DELTA_BYTE_ARRAY(7),
