...
public class MultiStatementsSqlSplitter {
	...
	// Adapted from spark <code>SparkSQLCLIDriver.splitSemiColon()</code> 
	// ... itself adapted from Hive 2.3's <code>CliDriver.splitSemiColon()</code>
	...
	public static List<TextWithLineNumber> splitSemiColon(String text) {
		...
		// computed helper.. reeval when changing either insideSimpleComment or bracketedCommentLevel 
		boolean insideComment = insideSimpleComment || bracketedCommentLevel > 0;

		final int textLen = text.length();
		for (int index = 0; index < textLen; index++) {
			// Checks if we need to decrement a bracketed comment level; the last character
			// '/' of
			// bracketed comments is still inside the comment, so `insideBracketedComment`
			// must keep true
			...
			if (ch == '\'' && !insideComment) {
				...
				if (insideDoubleQuote || insideSingleQuote || insideComment) {
					...
					// Avoids to start a comment(--) within a quoted segment or already in a
					// comment.
				...
				if (!escape) {
					...
					insideComment = bracketedCommentLevel > 0; // reeval computed var
				}
				...
				if (insideSingleQuote || insideDoubleQuote) {
					...
					bracketedCommentLevel++;
					insideComment = true; // reeval
				}
			}
			// set the escape
			escape = (ch == '\\');

			isStatement = isStatement || (!insideComment && index > beginIndex && Character.isWhitespace(ch));
		}
		...
		// If there is an uncompleted statement or an uncompleted bracketed comment in
		// the end,
		// CLI should also pass this part to the backend engine, which may throw an
		// exception
