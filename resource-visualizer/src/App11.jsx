...
function AllocationChart({ nodes, allocations }) {
    ...
    // Calculate total height based on the maximum row index
    const maxRowIndex = Math.max(...nodesWithRows.map(n => n.rowIndex), 0);
    const totalHeight = (maxRowIndex + 1) * ROW_SPACING + TOP_MARGIN;

    return (
        <div>
            {/* Legend */}
            <div style={{ marginBottom: '20px', padding: '10px', backgroundColor: '#f5f5f5', borderRadius: '5px' }}>
                <h3 style={{ margin: '0 0 10px 0', fontSize: '14px' }}>Layout Summary:</h3>
                <p style={{ margin: '5px 0', fontSize: '12px' }}>
                    Nodes placed in {maxRowIndex + 1} row(s) based on time overlap detection
                </p>
                {Array.from(new Set(nodesWithRows.map(n => n.rowIndex))).sort().map(rowIdx => (
                    <div key={rowIdx} style={{ fontSize: '12px', margin: '2px 0' }}>
                        Row {rowIdx}: {nodesWithRows.filter(n => n.rowIndex === rowIdx).map(n => n.id).join(', ')}
                    </div>
                ))}
            </div>
    ...
    return (
        <div style={{ padding: 20, fontFamily: 'Arial, sans-serif' }}>
            ...
            <p style={{ color: '#666', marginBottom: '20px', fontSize: '14px' }}>
                ...
                Nodes with overlapping time periods are placed in different rows.
            </p>
